 ...
 */
	...
	define("AVAILABILITY_OFFLINE", "offline"); // 0
	define("AVAILABILITY_IDLE", "idle"); // 1
	define("AVAILABILITY_ONLINE", "online"); // 2
	
class OpenInviter_Base
	{
	protected $session_id;
	protected $curl;
	public $last_effective_url = false;
	...
	protected $lastURL = false;
	protected $logoutURL = false;
	 ...
	 */
	public function getElementDOM($string_bulk,$query,$attribute=false)
		{
		$search_val=array();
		$doc=new DOMDocument();
		...
		protected function searchElementDOM($string_bulk, $query, $attribute = false)
		{
		$search_val=array();
		$doc=new DOMDocument();
		libxml_use_internal_errors(true);
		...
		if ($attribute)
			foreach ($data as $node)
			{
				if ($node->getAttribute("name"))
					$search_val[$node->getAttribute("name")] = $node->getAttribute($attribute);
				else
					$search_val[]=$node->getAttribute($attribute);
			}
		else
			...
			{
				if ($node->getAttribute("name"))
					$search_val[$node->getAttribute("name")] = $node->nodeValue;
				else
				 $search_val[] = $node->nodeValue;
			}
		if (empty($search_val))
	...
	public function extractFormElements($res) {
		$formDoc=new DOMDocument();
		$post_elements = array();
		$formDoc->loadHTML($res);
		$xPath = new DOMXPath($formDoc);
		$form_nodeList = $xPath->query("//form");
		if ($form_nodeList && $form_nodeList->length != 0) {
			$form_node = $form_nodeList->item(0);
			$inputElements = $form_node->getElementsByTagName("input");
			foreach ($inputElements as $x => $inputElement)
				$post_elements[$inputElement->getAttribute("name")] = $inputElement->getAttribute("value");
		}
		$form_action = $this->getElementDOM($res,"//form", 'action');
		$form_action = htmlspecialchars_decode(urldecode($form_action[0]));
		return array("action" => $form_action, "post_elements" => $post_elements);
	}
	
	public function stripElementString($string_to_search,$string_start,$string_end)
	{		
		...
		$start = strpos($string_to_search,$string_start);
		$end = strpos($string_to_search,$string_end,$start) + strlen($string_end);
		
		return substr($string_to_search,0,$start) . substr($string_to_search,$end);
	}
	 ...
	 */
	public function getElementString($string_to_search,$string_start,$string_end, $offset_position = 0, $return_last_position = false)
	{		
		if (strpos($string_to_search,$string_start, $offset_position) === false)
			return false;
			
		if (strpos($string_to_search,$string_end) === false)
			return false;
			
		$start = strpos($string_to_search,$string_start, $offset_position) + strlen($string_start);
		...
		{
			return array("result" => $return, "last_position" => $end);
		}
		else
		{
			return $return;
	...
	protected function followLocation($result,$old_url)
		...
		if (empty($new_url)) {
			$new_url=trim($this->getElementString($result,"location: ",PHP_EOL));
			// echo "<pre>location: $new_url</pre>";
		} // else echo "<pre>Location: $new_url</pre>";
		if (!empty($new_url)) {
			if (strpos($new_url,'http')===false)
				...
				// echo "<br>followLocation:: new_url:: $new_url<br>";
				$this->lastURL = $new_url;
		}
	...
	public function init($session_id=false)
		...
		if ($this->settings['transport'] == 'curl')
			{
			$this->curl = curl_init();
			//curl_setopt($this->curl, CURLOPT_STDERR, STDOUT);
			curl_setopt($this->curl, CURLOPT_VERBOSE, true);
			
			curl_setopt($this->curl, CURLOPT_USERAGENT,(!empty($this->userAgent)?$this->userAgent:"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.1453.93 Safari/537.36"));
			curl_setopt($this->curl, CURLOPT_FOLLOWLOCATION, false);
			// echo "<br><br>Cookie File:: $file";
	...
	{
		// echo "<br><br>GET:: $url".($referer?"<br>REFERER:: $referer":"");
		$this->lastURL = $url;
		...
		if ($this->settings['transport']=='curl')
		{
			...
			$this->last_effective_url = curl_getinfo($this->curl, CURLINFO_EFFECTIVE_URL);
			/*
			echo "<pre>";
			...
			*/
			...
			if(substr($result, 0, 4) == 'HTTP')
				{
				$result = explode("\r\n\r\n", $result, 2);
				if(count($result) > 1) $result = $result[1];
	 ...
	 * @return mixed The request response or FALSE if the response if empty.
	 */
	public function post($url,$post_elements,$follow=false,$header=false,$referer=false,$headers=array(),$raw_data=false,$quiet=true)
	{
		// echo "<br><br>POST:: $url".($referer?"<br>REFERER:: $referer":"");
		$this->lastURL = $url;
		$flag=false;
		if ($raw_data)
		...
		if ($this->settings['transport']=='curl')
			...
			/*
			echo "<pre>";
			echo "last_effective_url: " . $this->last_effective_url . "\n";
			print_r(curl_getinfo($this->curl, CURLINFO_HEADER_OUT));
			echo "</pre>";
			*/
			if (false && $follow)
	...
	protected function returnVCards($contacts)
		{
		$returnedVCards=array();
		foreach($contacts as $contact) $returnedVCards[] = $this->returnVCard($contact, 0);
		return $returnedVCards;
		}

	protected function returnVCard($contact, $print=1)
		{
		require_once "../common/VDataEditor.class.php";
		$ed = new VDataEditor();
		if(empty($contact['N']) && !empty($contact['EMAIL'][0])) $contact['N'] = $contact['EMAIL'][0];
		if(empty($contact['N']) && !empty($contact['TEL'][0]))   $contact['N'] = $contact['TEL'][0];
		$contact_vcard = $ed->create("contacts", $contact);
		if($print) $this->printJson('object', $contact_vcard);
		return $contact_vcard;
		}

	protected function printJson($command, $contents, $additional=array())
		{
		/*
		// trick to fix the import of pictures
		$contents = str_replace("ENCODING=BASE64:", "ENCODING=BASE64:\r\n    ", $contents);
		$contents = str_replace("\r\nN:", "\r\n\r\nN:", $contents);
		// TODO remove this trick and fix the import in a cleaner way
		*/
		
		if (!isset($additional['n']))
		{
			if (isset($additional['email']))
				$additional['n'] = $additional['email'];
			else if (isset($additional['tel']))
				$additional['n'] = $additional['tel'];
		}
		
		if (isset($additional['status']))
		{
			// $additional['status'] = htmlspecialchars_decode(html_entity_decode($additional['status']), ENT_QUOTES);
			$additional['status'] = html_entity_decode($additional['status'], ENT_QUOTES, "UTF-8");
		}
	...
	protected function restartSessionCheckCancel()
		{
		/*session_write_close();
		session_start();
		return isset($_SESSION['CONTACTS_IMPORTER_REQUEST_CANCEL']);*/
		return false;
		}
