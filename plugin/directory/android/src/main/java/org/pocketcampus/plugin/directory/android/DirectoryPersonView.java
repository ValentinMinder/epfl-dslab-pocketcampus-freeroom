...
public class DirectoryPersonView extends PluginView implements IDirectoryView {
	...
	protected void handleIntent(Intent aIntent) {
		...
		if (aData != null && "/search".equals(aData.getPath())
				&& aData.getQueryParameter("sciper") != null) {
			mController.searchBySciper(this, aData.getQueryParameter("sciper"));
		} else if (aData != null && "/view".equals(aData.getPath())) {
			Person p = new Person();
			if (aData.getQueryParameter("firstName") != null)
				p.setFirstName(aData.getQueryParameter("firstName"));
			if (aData.getQueryParameter("lastName") != null)
				p.setLastName(aData.getQueryParameter("lastName"));
			if (aData.getQueryParameter("sciper") != null)
				p.setSciper(aData.getQueryParameter("sciper"));
			p.setEmail(aData.getQueryParameter("email"));
			p.setWeb(aData.getQueryParameter("web"));
			p.setPrivatePhoneNumber(aData
					.getQueryParameter("privatePhoneNumber"));
			...
			if (aData.getQueryParameter("OrganisationalUnit") != null)
				oU.add(aData.getQueryParameter("OrganisationalUnit"));
			p.setOrganisationalUnits(oU);
			if (aData.getQueryParameter("pictureUrl") != null)
				p.setPictureUrl(aData.getQueryParameter("pictureUrl"));
			displayPerson(p);
		}

		// Tracker
		// if(eventPoolId == Constants.CONTAINER_EVENT_ID)
		// Tracker.getInstance().trackPageView("directory");
		// else Tracker.getInstance().trackPageView("directory/" + eventPoolId +
		// "/subevents");
	}
	...
	private void displayPerson(final Person p) {
		...
		// R.layout.directory_list_header);
		MultiListAdapter adapter = new MultiListAdapter();

		Preparated<Person> pp = new Preparated<Person>(
				Arrays.asList(new Person[] { p }), new Preparator<Person>() {
					public int[] resources() {
						return new int[] {
								R.id.directory_person_details_picture,
								R.id.directory_person_details_name,
								R.id.directory_person_details_affiliation,
								R.id.directory_person_details_expanded_affiliation };
					}
					...
					public Object content(int res, final Person e) {
						...
						case R.id.directory_person_details_name:
							return new LazyAdapter.Actuated(DirectoryController
									.getFullName(e),
									new LazyAdapter.Actuator() {
										int count = 10;
										...
										public void triggered() {
											...
											if (count == 0)
												startActivity(new Intent(
														Intent.ACTION_VIEW,
														Uri.parse("http://people.epfl.ch/cgi-bin/people/getPhoto?id="
																+ e.getSciper()
																+ "&show=1")));
										}
									});
		...
		adapter.addSection(new LazyAdapter(this, pp.getMap(),
				R.layout.directory_person_details, pp.getKeys(), pp
						.getResources())
				.setImageOnFail(R.drawable.sdk_empty_person)
				.setNoImage(R.drawable.sdk_empty_person)
				.setImageForEmptyUri(R.drawable.sdk_empty_person));
		...
		if (p.isSetOfficePhoneNumber() || p.isSetPrivatePhoneNumber()) {
			...
			String pN = (p.isSetOfficePhoneNumber() ? p.getOfficePhoneNumber()
					: p.getPrivatePhoneNumber());
			actions.add(new PersonInteraction(R.drawable.directory_phone, pN,
					Uri.parse("tel:" + pN), "Call"));
		}
		if (p.isSetEmail()) {
			// ic_dialog_email, ic_menu_send
			actions.add(new PersonInteraction(R.drawable.directory_mail, p
					.getEmail(), Uri.parse("mailto:" + p.getEmail()),
					"SendEmail"));
		}
		if (p.isSetHomepages()
				&& p.getHomepages().containsKey("Personal profile")) {
			// ic_dialog_info, ic_menu_info_details
			String homepage = p.getHomepages().get("Personal profile");
			actions.add(new PersonInteraction(R.drawable.directory_web,
					homepage, Uri.parse(homepage), "ViewWebsite"));
		} else if (p.isSetWeb()) {
			actions.add(new PersonInteraction(R.drawable.directory_web, p
					.getWeb(), Uri.parse(p.getWeb()), "ViewWebsite"));
		}
		if (p.isSetOffice()) {
			// ic_dialog_map, ic_menu_compass, ic_menu_mapmode
			Uri.Builder builder = new Uri.Builder();
			builder.scheme("pocketcampus")
					.authority("map.plugin.pocketcampus.org")
					.appendPath("search")
					.appendQueryParameter("q", p.getOffice());
			actions.add(new PersonInteraction(R.drawable.directory_map, p
					.getOffice(), builder.build(), "ViewOffice"));
		}

		Preparated<PersonInteraction> pD = new Preparated<PersonInteraction>(
				actions, new Preparator<PersonInteraction>() {
					public int[] resources() {
						return new int[] { R.id.directory_person_action_icon,
								R.id.directory_person_action_text };
					}
					...
					public void finalize(Map<String, Object> map,
							PersonInteraction item) {
						map.put(MAP_KEY_ACTION_URI, item);
					}
				});
		adapter.addSection(new LazyAdapter(this, pD.getMap(),
				R.layout.directory_person_action, pD.getKeys(), pD
						.getResources())
				.setImageOnFail(R.drawable.sdk_empty_person)
				.setNoImage(R.drawable.sdk_empty_person)
				.setImageForEmptyUri(R.drawable.sdk_empty_person));
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (o instanceof Map<?, ?>) {
					...
					if (obj != null && obj instanceof PersonInteraction) {
						trackEvent(((PersonInteraction) obj).tracking,
								((PersonInteraction) obj).text);
						Intent i = new Intent(Intent.ACTION_VIEW,
								((PersonInteraction) obj).uri);
						startActivity(i);
					}
				} else {
		...
		addActionToActionBar(new Action() {
			...
			public String getDescription() {
				return getString(R.string.directory_add_to_contacts);
			}
		});
	...
	public void ambiguousQuery() {
		Toast.makeText(getApplicationContext(),
				getString(R.string.directory_ambiguous_query),
				Toast.LENGTH_SHORT).show();
		finish();
	}
	...
	private static String getOrgUnitsString(Person p) {
		...
		for (String k : p.getRoles().keySet()) {
			...
			roles.add("<b>&bull; <i>"
					+ r.getLocalizedTitle()
					+ "</i></b> &mdash; <i>"
					+ r.getExtendedLocalizedUnit()
					+ " (<a href=\"pocketcampus://directory.plugin.pocketcampus.org/query?q="
					+ k + "\">" + k + "</a>)</i>");
		}
