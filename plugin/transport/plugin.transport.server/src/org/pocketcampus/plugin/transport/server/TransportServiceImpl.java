...
public class TransportServiceImpl implements TransportService.Iface {
	...
	public QueryDepartureResult nextDepartures(String IDStation)
		...
		try {
			nextDepartures = convertSchToPC(mSbbProvider.queryDepartures(
	...
	public QueryConnectionsResult connections(String from, String to)
		...
		try {
			connections = convertSchToPC(mSbbProvider.queryConnections(fromLoc,
	...
	// STATIONS DEPARTURS
	private List<StationDepartures> convertSchStaDepToPC(
	...
	private StationDepartures convertSchToPC(
	...
	private List<LineDestination> convertSchLiDesToPC(
	...
	private LineDestination convertSchToPC(
	...
	// DEPARTURES
	private List<Departure> convertSchDepToPC(
	...
	private NearbyStatus convertSchToPC(
	...
	private Location convertSchToPC(de.schildbach.pte.dto.Location s) {
		if (s != null)
			return new Location(convertSchToPC(s.type), s.id, s.lat, s.lon,
					s.place, s.name);
		else
			return null;
	}
	...
	private Connection convertSchToPC(de.schildbach.pte.dto.Connection sc) {
		return new Connection(sc.id, sc.departureTime.getTime(),
				sc.arrivalTime.getTime(), convertSchToPC(sc.from),
				convertSchToPC(sc.to));
	}

	private List<Connection> convertSchConToPC(
	...
	private List<Part> convertSchPartsToPC(
	...
	private Part convertSchToPC(de.schildbach.pte.dto.Connection.Part sf) {
		Part part = new Part(convertSchToPC(sf.departure),
				convertSchToPC(sf.arrival), convertSchPointsToPC(sf.path));
	...
	private Stop convertSchToPC(de.schildbach.pte.dto.Stop s) {
		return new Stop(convertSchToPC(s.location), s.position,
				s.time.getTime());
	}
