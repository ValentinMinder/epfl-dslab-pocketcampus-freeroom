...
public class EventsMainView extends PluginView implements IEventsView {
	...
	protected void handleIntent(Intent aIntent) {
		eventPoolId = EventsConstants.CONTAINER_EVENT_ID;
	...
	private void updateActionBar() {
		...
		// if(eventPoolId == EventsConstants.CONTAINER_EVENT_ID) {
		...
		if (!thisEventPool.isDisableFilterByCateg()) {
			final Map<Integer, String> subMap = subMap(EventsConstants.EVENTS_CATEGS, categsInRS);
			if (subMap.size() > 0) {
				addActionToActionBar(new Action() {
					public void performAction(View view) {
		...
		if (!thisEventPool.isDisableFilterByTags()) {
			final Map<String, String> subMap = subMap(EventsConstants.EVENTS_TAGS, tagsInRS);
			if (subMap.size() > 0) {
				addActionToActionBar(new Action() {
					public void performAction(View view) {
		...
		if (happeningNow)
			return;
		if (eventPoolId == EventsConstants.CONTAINER_EVENT_ID) { // settings thingy
			addActionToActionBar(new Action() {
				@Override
				public void performAction(View view) {
					showSingleChoiceDialog(EventsMainView.this, EventsConstants.EVENTS_PERIODS, getString(R.string.events_choose_period),
	...
	private void updateDisplay(boolean saveScroll) {
		...
		for (int i : categList) {
			...
			adapter.addSection(EventsConstants.EVENTS_CATEGS.get(i), lazyAdapter);
		}
	...
	// showSingleChoiceDialog(EventsMainView.this, EventsConstants.EVENTS_PERIODS,
