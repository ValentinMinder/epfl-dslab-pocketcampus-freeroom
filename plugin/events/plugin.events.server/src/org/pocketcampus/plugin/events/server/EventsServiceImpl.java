...
public class EventsServiceImpl implements EventsService.Iface {
		...
		Runnable sender = new Runnable() {
			public void run() {
				try {
					...
					rs = stm.executeQuery();
					if (rs.next())
						template = new EmailTemplateInfo(rs.getLong(1), rs.getString(2), rs.getString(3), (rs.getString(4) == null ? null : arrayToList(rs.getString(4)
								.split("[,]"))));
					rs.close();
					stm.close();
					if (template == null) {
	 ...
	 * edicOpenHouse.setEventDetails(
	 * "The EDIC Open House is a gathering of prospective PhD students (admitted from the January 15th application deadline) and the I&C School community, to learn about our program, the university and the charming Lake Geneva Region. "
	 * );
