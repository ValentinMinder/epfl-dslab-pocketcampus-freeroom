 ...
 *
 */
public class CourseSectionsRequest extends Request<EdXController, Iface, EdxReq, EdxResp> {

	private IEdXView caller;
	
	public CourseSectionsRequest(IEdXView caller) {
		this.caller = caller;
	}
	...
	protected void onResult(EdXController controller, EdxResp result) {
		if(result.getStatus() == 200) {
			((EdXModel) controller.getModel()).setCourseSections(result.getCourseSections());
