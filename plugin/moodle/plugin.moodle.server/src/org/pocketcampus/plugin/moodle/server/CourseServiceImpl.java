...
public final class CourseServiceImpl implements CourseService {
    ...
    public MoodleCourseSectionsResponse2 getSections(final MoodleCourseSectionsRequest2 request) {
        ...
        for (final JsonSection section : sections) {
            if (section.visible == VISIBLE) {
                ...
                if (!isNullOrWhitespace(section.summary) && section.summaryformat == SUMMARY_FORMAT_HTML) {
    ...
    /** Checks whether the specified string is null or only composed of space characters. */
    private static boolean isNullOrWhitespace(final String str) {
        if (str == null) {
            return true;
        }
