...
public class CloudPrintModel extends PluginModel implements ICloudPrintModel {
	...
	private static CloudPrintMultiPageConfig stringToMultiPage(String s) {
		if("_".equals(s))
			return null;
		String [] ss = s.split("[_]");
		return new CloudPrintMultiPageConfig(CloudPrintNbPagesPerSheet.findByValue(Integer.parseInt(ss[0])), CloudPrintMultiPageLayout.findByValue(Integer.parseInt(ss[1])));
	}
