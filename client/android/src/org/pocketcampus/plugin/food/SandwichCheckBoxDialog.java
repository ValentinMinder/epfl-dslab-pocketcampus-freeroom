...
public class SandwichCheckBoxDialog {
	...
	private void valid() {
		...
		// IllegalArgumentException("ServerAPI cannot be null ");
	}
	...
	private void createAlertDialog(final boolean modify) {
				...
				new DialogInterface.OnMultiChoiceClickListener() {
					...
					public void onClick(DialogInterface dialog, int item,
						...
						if (!modify) {
							...
							confirmationChangeCheckBox(nameSandwichList[item]
									.toString(), item, isChecked, dialog);
		...
		if (!modify /* dialog box no modify */) {
			builder.setNeutralButton(R.string.food_sandwich_button_modify,
	...
	private void confirmationChangeCheckBox(final String nameSandwich,
				...
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						...
						new Thread() {
							public void run() {
								/*
								 * try { if(sApi.ping()){ //envoie les
								 * information au serveur.
								 * sApi.setSandwichAvailability
								 * (sandwichList_.get(item), isChecked);
								 * handler.sendEmptyMessage(1); } else {
								 * handler.sendEmptyMessage(0); } } catch
								 * (ServerException e) {
								 * handler.sendEmptyMessage(-4);
								 * e.printStackTrace(); dialog2.dismiss(); }
								 */
