package org.pocketcampus.android.platform.sdk.ui.adapter;

import java.util.List;

import org.pocketcampus.R;
import org.pocketcampus.android.platform.sdk.ui.element.ElementDimension;

import android.content.Context;
import android.widget.ArrayAdapter;

/**
 * TODO make customizable
 * @author Oriane
 */
public abstract class AbstractMinimalArrayAdapter extends ArrayAdapter<Object> {
	protected static int mLayoutResourceId = R.layout.sdk_list_entry_minimal;
	protected static int mTextViewResourceId = R.id.sdk_list_minimal_entry_text;

	private ElementDimension mDimension = ElementDimension.NORMAL;
	
	public AbstractMinimalArrayAdapter(Context context, List<? extends Object> items) {
		super(context, mLayoutResourceId, mTextViewResourceId, items.toArray());
	}
	
	public void setDimension(ElementDimension dimension) {
		mDimension = dimension;
	}
	
}
