...
public class TequilaService
{
   ...
   * @param urlacces the url where the user is redirected after successful authentification.
   * @return the corresponding Tequila session key.
   * @throws IOException if the connection with the Tequila server cannot be made.
   */
  public String createRequest(ClientConfig config, String urlacces) throws IOException
  {
    // the parameters
    ...
    appendParameter(parameters, "authstrength", config.getAuthstrength());
    appendParameter(parameters, "urlacces", urlacces);
  ...
  public TequilaPrincipal validateKey(ClientConfig config, String sessionKey) throws IOException
  {
    ...
    // if the response code is not 200, we have an invalid key
    if (connection.getResponseCode() != 200)
    {
      throw new SecurityException("Invalid session key : " + sessionKey + ". Response was : " + connection.getResponseCode() + " " + connection.getResponseMessage());
    }
