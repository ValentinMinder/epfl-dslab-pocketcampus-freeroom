...
#WARNING : add_folder method modified to, in addition to files, not add folders that match the exludes RegEx list
...
class XcodeProject(PBXDict):
	...
	def add_folder(self, os_path, parent=None, excludes=None, recursive=True, create_build_files=True):
		...
		for (grp_path, subdirs, files) in os.walk(os_path):
			...
			if folder_name.startswith('.') or [m for m in excludes if re.match(m,grp_path)]:
