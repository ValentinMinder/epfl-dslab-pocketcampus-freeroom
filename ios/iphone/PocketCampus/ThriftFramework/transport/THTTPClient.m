...
- (void) setupRequest
{
  ...
  if (mTimeout) {
    [mRequest setTimeoutInterval: (NSTimeInterval)mTimeout];
  }
}
...
- (void) flush
{
  ...
  NSError * error;
    //NSLog(@"BEFORE sendSynchronousRequest ((%p) :timeout : %lf)",mRequest, mRequest.timeoutInterval);
  NSData * responseData = [NSURLConnection sendSynchronousRequest: mRequest returningResponse: &response error: &error];
    //NSLog(@"AFTER sendSynchronousRequest");
