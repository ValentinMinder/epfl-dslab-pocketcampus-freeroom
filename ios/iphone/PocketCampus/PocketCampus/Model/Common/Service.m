...
@synthesize delegate, thriftServiceClient, timedOut, delegateDidReturnSelector, delegateDidFailSelector, serviceClientSelector, returnType;
...
- (void)checkPrimariesAndScheduleTimeoutTimer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        timeoutTimer = [[NSTimer scheduledTimerWithTimeInterval:[Service requestTimeoutInterval] target:self selector:@selector(didTimeout) userInfo:nil repeats:NO] retain];
    });
}
...
- (void)main {
    ...
    @catch (NSException *exception) {
        NSLog(@"-> ServiceRequest EXCEPTION caught in main : %@", exception);
        @try {
            
            if (self.timedOut) {
                @throw [NSException exceptionWithName:@"service request returned after timeout" reason:@"timeout" userInfo:nil];
            }
